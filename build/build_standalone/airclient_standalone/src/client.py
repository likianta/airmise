...
def main(host: str, frontend_port: int, backend_port: int) -> None:
    ...
    if host == '47.102.108.149':
        # fix port via frp service
        # noinspection PyProtectedMember
        client.master._send(air.const.INTERNAL, 'get_socket_port')
        # noinspection PyProtectedMember
        old, new = client.master.socket.port, client.master._recv()
        print(f'fix socket port via frp service: {old} -> {new}', ':r2')
        # noinspection PyProtectedMember
        client._socket.port = new
